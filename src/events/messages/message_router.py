...
async def setup_message_handler(bot):
    ...
    async def on_message(message):
        try:
            ...
            logging.debug(f"Current Sunraku state: {sunraku_state}")

            if not get_toggle_bot():
                if not any(message.content.startswith(cmd) for cmd in allowed_commands):
                    logging.info("Bot is toggled off. Ignoring message.")
                    return

            ctx = await bot.get_context(message)
            if ctx.command:
                logging.info(f"Processing command: {message.content} from {message.author}")
            ...
            logging.error(f"Error processing message: {message.content} from {message.author}: {str(e)}")
            raise e
